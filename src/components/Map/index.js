...
class MapContainer extends Component {
    ...
    render() {
        ...
        const markers = experiences.map(experience => {
            if (experience.media && experience.media.metadata && experience.media.metadata.gps_latitude) {
                const position = {
        ...
        return (
            <Fragment>
                <Map className="profile-map" google={this.props.google} initialCenter={{lat: 50, lng: 4}} zoom={3}>
                    {markers}
                    <InfoWindow marker={this.state.activeMarker} visible={this.state.showingInfoWindow}>
                        <div>
                            <h1>{this.state.selectedPlace.title}</h1>
                            <img style={{width: '100%', height: '100%'}} src={this.state.selectedPlace.image} />
                        </div>
                    </InfoWindow>
                </Map>
            </Fragment>
